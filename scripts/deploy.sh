...
if [ "$TARGET" = "local" ]; then
  ...
  # Using -p 0:3000 tells Docker to bind to any available port on the host
  echo "🐳 Starting container with dynamic port allocation..."
  CONTAINER_ID=$(docker run -d -p 0:3000 --name lcc-live lcc.live:latest)
  ...
  if docker ps | grep -q lcc-live; then
    ...
    HOST_PORT=$(docker port lcc-live 3000 | cut -d: -f2)
  ...
  fly deploy --local-only --image lcc.live:latest --release-command-timeout=90s
